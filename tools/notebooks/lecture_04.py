...
"""),
        ...
        prompt(
            ...
            check="Restart-and-run-all before you believe any notebook, including your own. It is the only test of this that exists."),
        code('''
...
'''),
        ...
        prompt(
            ...
            check="assert the shape, that Survived exists, and that it holds only 0/1. Delete `datasets/` and re-run. If the cell cannot rebuild its own input from nothing, it is not reproducible, it is just cached."),
        code('''
...
'''),
        prompt(
            ...
            check="assert the exact dictionary, so the numbers below are pinned to a known file. Compare rows before and after any drop. If a one-line clean removes most of your data, the line is the problem, not the data."),
        code('''
...
"""),
        prompt(
            ...
            check="assert 342 survivors. Every classification notebook should print its base rate above the first model. It costs one line and it is the denominator of every claim below it."),
        code('''
...
"""),
        prompt(
            ...
            check="assert the two extremes, above 0.95 and below 0.15. If a hand-written rule matches your model, you have not built a model, you have built an expensive way to write down that rule."),
        code('''
...
"""),
        prompt(
            ...
            check="assert the title set is exactly the five expected, and that no engineered column is null. Value_counts on every categorical you create, before encoding it. A level with n=1 is a memorised passenger wearing a coefficient."),
        code('''
def engineer(d):
    d = d.copy()
...
"""),
        prompt(
            ...
            check="assert the sizes, that the indices are disjoint, and that the two base rates agree to within a point. Assert on the two base rates, not just the two sizes. Sizes are right even when the split is badly unbalanced."),
        code('''
...
"""),
        prompt(
            ...
            check="assert it equals 0.666 — this number is quoted for the rest of the session. Always report the anchor beside the model. A log loss of 0.45 sounds like nothing until you see that doing no work at all scores 0.666."),
        code('''
p = y_train.mean()
...
"""),
        prompt(
            ...
            check="assert no NaN survives, and print how many raw columns became how many model columns. If any preprocessing step is fitted outside a Pipeline, your cross-validation score is optimistic. There is no version of that which is fine."),
        code('''
...
"""),
        prompt(
            ...
            check="assert the output is hard 0/1 labels, which is the defect made visible rather than a test of correctness. Whenever you see `.predict()` on a problem with unequal costs, ask what threshold it used. If the answer is 'the default', the threshold is unowned."),
        code('''
...
"""),
        prompt(
            ...
            check="sanity-check with a degenerate threshold. At t=0.98 nearly everyone is flagged; if your cost is LOWEST there, the comparison is backwards."),
        code('''
...
"""),
        prompt(
            ...
            check="assert the cost-chosen rule is never beaten on cost, and that it is WORSE on accuracy — both directions matter. Count the seeds where the sign goes your way and report that count. '20 of 20' is an argument; a mean difference on its own is not."),
        code('''
def one_seed(seed):
...
"""),
        prompt(
            ...
            check="assert ten folds were returned and that the model beats the 0.666 anchor. Print the per-fold numbers, not only the mean. Ten folds whose spread is 0.08 do not support a claim about a difference of 0.01."),
        code('''
...
"""),
        prompt(
            ...
            check="assert the name list and the coefficient vector are the same length — a mismatch here silently mislabels every row of the table. `get_feature_names_out()` beside `coef_`, always zipped, never assumed to line up by memory of the column order."),
        code('''
with warnings.catch_warnings():
    warnings.simplefilter("ignore")
...
"""),
        prompt(
            ...
            check="assert deck T has exactly one passenger. For every large coefficient on a categorical level, look up how many rows carry that level. Large weight plus tiny n is memorisation."),
        code('''
...
"""),
        prompt(
            ...
            check="assert 29 columns and rank 23. Rank against column count, on the design matrix WITH the intercept. The gap is the number of directions your coefficients can move in without changing a prediction."),
        code('''
...
"""),
        prompt(
            ...
            check="5 one-hot blocks + 1 engineered sum must account for the whole deficiency. When you build a feature as a sum of others, you have created an exact dependence. Combining features is fine; combining them ADDITIVELY alongside their own parts is not."),
        code('''
...
"""),
        prompt(
            ...
            check="assert the logits agree to under 1e-9 AND that the vectors genuinely differ — one assert without the other proves nothing. The pair of asserts. Numerically identical predictions from provably different parameters is what non-identifiability looks like in code."),
        code('''
...
"""),
        prompt(
            ...
            check="assert columns equals rank equals 23 — full rank. The condition number, before and after: 2.6e+16 to under a hundred. It moves by fourteen orders of magnitude, which is not something you can misread."),
        code('''
...
'''),
        prompt(
            ...
            check="assert Sex_female is gone — one level of each block must have been dropped. State the reference levels out loud. A coefficient table with drop='first' and no statement of what was dropped cannot be read by anyone, including you in a month."),
        code('''
...
"""),
        prompt(
            ...
            check="assert the shape is 712 and that every value lies in [0, 1]. Print n for each bin beside the rates. A bin with n=6 whose observed rate is 0.17 is one passenger, not a calibration failure."),
        code('''
with warnings.catch_warnings():
    warnings.simplefilter("ignore")
...
"""),
        prompt(
            ...
            check="assert the cost-optimal rule flags MORE people than 0.5 — a 10:1 ratio should buy more escorts, and if it buys fewer the sign is inverted somewhere. Whenever a rule outputs a count of PEOPLE, read the count. 605 out of 712 should stop you regardless of what the cost column says."),
        code('''
...
'''),
        prompt(
            ...
            check="a decision plot should show the alternative you are arguing against. Otherwise it is an illustration, not an argument."),
        code('''
...
"""),
        prompt(
            ...
            constraint="the rule must be a RANKING, not a cut-off — take the "
                       ...
                       "happens to have",
            check="exactly 80 escorts are assigned, and the unconstrained rule  asks for more than 80, so the constraint is shown to bind  rather than assumed to. Assert the escort count equals CREW exactly. A rule that  produces 74 escorts because that is where a grid point fell  is not a capacity rule, it is a threshold rule that got  lucky — and it wastes six crew."
        ),
        prompt(
            label="the policy you can actually staff",
            input="the same out-of-fold probabilities, and CREW = 80",
            ...
            check="exactly 80 escorts are assigned, and the unconstrained rule asks for more than 80, so the constraint is shown to bind rather than assumed to. Assert the escort count equals CREW exactly. A rule producing 74 escorts because that is where a grid point fell is a threshold rule that got lucky, and it wastes six crew."),
        code('''
...
"""),
        prompt(
            ...
            check="assert training error falls at every degree while held-out error does not — the two asserts together are the experiment. 22 columns at degree 1 becomes 143 at degree 5, on 712 rows. Count your columns against your rows before you interpret anything."),
        code('''
...
'''),
        prompt(
            ...
            check="degrees 1 and 2 differ by under a hundredth, and the fold-to-fold spread is far larger. They are tied, and reporting 2 as the winner is reporting noise."),
        code('''
...
"""),
        prompt(
            ...
            check="assert degree 1 converges and degree 4 does not, so the transition is pinned. Two failures with the same shape: rank 23 of 29 means the minimiser is not UNIQUE, non-convergence at degree 4 means it does not EXIST. Both are about the optimisation problem, not the passengers."),
        code('''
conv = {}
for d in DEGREES:
...
"""),
        prompt(
            ...
            check="assert the best degree is 1 or 2 — the two simplest models are tied at the top, and an assert that accepted only one of them would be asserting noise. Photograph this output beside the four numbers you wrote down in section 5. The next lecture is scored against both."),
        code('''
