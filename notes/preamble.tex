...
%
% NOT `breakable`. A box split across a page break loses the thing that makes
% it a box: you turn the page and the argument has no lid on it, and the reader
% cannot tell where it ended. tcolorbox is unbreakable by default; these four
% were opted in, and are now opted back out. The cost is that a box taller than
% the text block overflows instead of splitting, so `make notes` checks for
% Overfull \vbox and there are none -- if you add a long box and that check
% fires, split the box in the source rather than making it breakable again.
\newtcolorbox{keybox}[1][]{
  enhanced, colback=aimlsoft, colframe=aimlprimary,
...
\newtcolorbox{failbox}[1][]{
  enhanced, colback=white, colframe=aimlaccent,
...
\newtcolorbox{derivbox}[1][]{
  enhanced, colback=white, colframe=aimlmath,
...
\newtcolorbox{trybox}[1][]{
  enhanced, colback=white, colframe=aimlsuccess,
...
% \notestitlefor{9}{II}{Neural networks, from the perceptron up}{Fashion-MNIST}{4}
% For the lectures Geron DOES cover. The Part V block below claims these notes
% are the primary source, which is true for 19-22 and false everywhere else --
% saying it on all twenty-four would tell a student the textbook is optional.
\newcommand{\notestitlefor}[5]{%
  \thispagestyle{empty}
  \begin{flushleft}
    ...
    {\color{aimlmuted}\large Lecture #1 \quad$\cdot$\quad Part #2 \quad$\cdot$\quad
     Lecture notes}\\[3mm]
    {\Huge\bfseries\color{aimlprimary}\baselineskip=1.15\baselineskip #3\par}\vspace{4mm}
    {\color{aimlmuted}\large Dataset: #4}\\[3mm]
    {\color{aimlrule}\rule{\textwidth}{0.8pt}}
  \end{flushleft}
  \vspace{2mm}
  \begin{keybox}[title={How to use these notes}]
    The primary text for this lecture is G\'eron, #5. These notes are not a
    replacement for it and do not repeat it: they are the lecture's own
    argument written out at length --- the derivation done slowly, the numbers
    with the runs that produced them, and the reasoning between one slide and
    the next that a deck can only gesture at. Read the chapter for the method;
    read these for why the lecture makes the choices it does. Every figure
    quoted here is printed by this lecture's notebook, and the course checks
    that mechanically rather than trusting it.
  \end{keybox}
  \vspace{1mm}
