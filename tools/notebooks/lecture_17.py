...
"""),
        prompt(
            ...
            check="whenever a library hands you a variable-length result, ask what decided the length. Two defaults decided this one and neither was mentioned in the prompt or the output."),
        code('''
...
"""),
        prompt(
            ...
            check="a count that hits a round number like exactly 100 is almost always a cap rather than a measurement. Look it up before interpreting it."),
        code('''
...
"""),
        prompt(
            ...
            check="assert the kept count never exceeds the returned count, and assert the MAE beats the one-box-per-image baseline. The signed bias beside the absolute one. A MAE of 3.00 that is entirely over-counting is a different system from one that is balanced, and only the signed figure distinguishes them."),
        code('''
...
"""),
        prompt(
            ...
            check="try to write down a number for 'how wrong is that box'. You cannot, and neither can the metric you committed to."),
        code('''
...
"""),
        prompt(
            ...
            check="if your formula does something odd for the DISJOINT pair, do not fix it. That is the interesting case, and the next lecture is largely about it."),
        code('''
