...
\begin{failbox}[title={Failure condition --- skip the correction and popularity bias is in the gradient}]
...
\begin{failbox}[title={Failure condition --- the random split is a leak, and it looks like a convention}]
...
\midrule
Temporal $\cdot$ factorisation & 0.0926 & 0.6792 \\
Temporal $\cdot$ popularity    & 0.0404 & 0.4864 \\
Random $\cdot$ factorisation   & 0.2132 & 0.8102 \\
Random $\cdot$ popularity      & 0.0879 & 0.6186 \\
\bottomrule
\end{tabular}
\end{center}

The same model on the same data scores 0.0926 and 0.8102 --- a factor of 8.8.
...
\midrule
Temporal $\cdot$ factorisation & 0.0464 & 0.4160 \\
Temporal $\cdot$ popularity    & 0.0196 & 0.2656 \\
Random $\cdot$ factorisation   & 0.1204 & 0.5630 \\
Random $\cdot$ popularity      & 0.0462 & 0.3709 \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Ratio} \\
\midrule
Factorisation & 0.0926 & 0.6792 & $\times 7.3$ \\
Popularity    & 0.0404 & 0.4864 & $\times 12.0$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{failbox}[title={Failure condition --- so sampled metrics are not merely noisy}]
...
Temporal, sampled 100    & $\times 1.40$ \\
Random, sampled 100      & $\times 1.31$ \\
\bottomrule
\end{tabular}
\end{center}

The absolute numbers spanned a factor of 8.8; the ratio to the baseline spans
from $\times 1.31$ to $\times 2.42$.
...
\begin{failbox}[title={Failure condition --- nothing in the objective notices}]
...
\emph{``A paper reports HR@10 of 0.8102 for its new model, against 0.0404 for a
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item Under one protocol, popularity itself reaches 0.6186 --- larger than
...
from 0.0926 to 0.8102 on one unchanged model, and it lets a popularity list
