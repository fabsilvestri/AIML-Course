...
\begin{failbox}[title={Failure 1 --- more features than instances}]
...
\begin{failbox}[title={Failure 2 --- collinear features}]
...
\begin{failbox}[title={The condition, stated generally}]
...
\begin{failbox}[title={The habit this is training}]
...
\begin{failbox}[title={Two things to notice, and neither is the improvement}]
...
\begin{failbox}[title={And in cross-validation it can vanish entirely}]
...
\begin{failbox}[title={The temptation, and why it is fatal}]
...
\begin{failbox}[title={These two numbers cannot be compared}]
...
alone, or ``no'' alone, is half the marks.
