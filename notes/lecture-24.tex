...
At rank 1 the recovery is complete to within one query out of 200 --- which is
noise, not a win over the human writers. At rank 10 it is \emph{not} complete,
and that gap is the real finding.
...
\begin{failbox}[title={Three things not to claim}]
...
\begin{failbox}[title={Failures of the join, visible from neither half}]
...
\begin{failbox}[title={What this does not fix}]
...
\midrule
Text query to image, joint space & 72.5\% & 72.5\% (untouched) \\
