...
\begin{itemize}[leftmargin=*,nosep]
  \item The Hessian is $X^{\mathsf T}X$, so the curvature along a feature goes as the \emph{square} of its scale --- multiply a column by ten and its curvature rises a hundredfold, which is how one column comes to dominate
...
\begin{itemize}[leftmargin=*,nosep]
  \item Each one-hot block sums to 1 in every row, and so does the intercept column --- five categoricals, five exact dependencies
  \item The sixth is \code{FamilySize = SibSp + Parch + 1}, an identity rather than a correlation. Dropping a level alone leaves rank 23 of 24, because the dependency you wrote yourself survives it
\end{itemize}
\end{solution}
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item They can part only on a pair of candidate splits whose children are incomparable in that ordering and whose weighted impurities differ by less than 0.055 --- a close call deep in the tree, never at the root
\end{itemize}
\end{solution}
...
\end{itemize}
\end{solution}
\question{Twenty trees are refit on 90\% subsamples of one training set, so any two of them share about 80\% of their rows --- and they still give visibly different trees. Does this make the model unreliable? Answer, with a reason.}{5}
\begin{solution}[title={Solution}]
...
\begin{itemize}[leftmargin=*,nosep]
  \item Accuracy averages over 12,000 patches, and an average hides substitutions: a tree that loses one patch and gains another scores the same
  \item The instability is deep in the tree, not at the root --- the root feature is Elevation in all twenty refits --- so the single decision you would be asked to explain is the part that does not reproduce
\end{itemize}
\end{solution}
...
\begin{itemize}[leftmargin=*,nosep]
  \item A single accuracy averages the class away; per-class recall costs nothing and makes the failure visible, which is the only response available before anyone has decided anything
  \item Whether that class is worth other classes' recall is the agency's decision, not a machine learning one --- and reweighting, the obvious next step, is a different model answering a different question
\end{itemize}
\end{solution}
...
\begin{solution}[title={Solution}]
Speed, not a lower floor. Not searching for the best threshold is what makes them fast; measured against the forest their $\rho$ is \emph{higher} (0.067 against 0.043), and they stay competitive because the two mechanisms are substitutes rather than additions.
\begin{itemize}[leftmargin=*,nosep]
  \item Extra-trees without a bootstrap land at $\rho = 0.067$, near bagging; with one they land at 0.043, exactly where the forest is --- random thresholds substitute for the bootstrap rather than adding to it
  \item Their individual members are \emph{better}, not worse --- 77.8\% against 73.6\% --- because a random threshold on every feature beats an optimal threshold on a subsample of them
\end{itemize}
\end{solution}
...
\begin{itemize}[leftmargin=*,nosep]
  \item A deeper network needs more data to pay for its extra parameters, so a sweep run on a fifth of the data systematically prefers the shallow ones --- and never finds out
  \item It is a measurement of one sweep at one size, not a rule. Reporting it as a general recommendation is the error
\end{itemize}
\end{solution}
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item A confident network reaching logits of 100 is completely ordinary, and $e^{100}$ is not representable --- the sum becomes infinite and the ratio a NaN
\end{itemize}
\end{solution}
...
\end{itemize}
\end{solution}
\question{Enlarging a word vocabulary drives the share of unseen distinct test words down, but even the full 79,003-word vocabulary leaves 42.8\% of them unseen. Explain why a larger vocabulary is not the fix, and name what is.}{4}
\begin{solution}[title={Solution}]
A word vocabulary is closed and language is not. Subword tokenisation.
\begin{itemize}[leftmargin=*,nosep]
  \item Two words in five are seen exactly once, so more capacity buys mostly those --- and the next review can still contain a word nobody has written yet
  \item Starting from characters covers every string by construction, so the floor is zero rather than 42.8\%
\end{itemize}
\end{solution}
...
\begin{solution}[title={Solution}]
$\operatorname{Var}(s) = d_k$, so $s$ has standard deviation $\sqrt{d_k}$; without the division the softmax saturates and the gradient vanishes.
\begin{itemize}[leftmargin=*,nosep]
  \item A sum of $d_k$ independent zero-mean products, each of variance 1, has variance $d_k$ --- at $d_k = 64$ typical scores are already of order 8
  \item Large logits make the distribution nearly one-hot, so almost no gradient flows
\end{itemize}
\end{solution}
\question{A decoder must not attend to positions after the one it is predicting. State the mechanism that enforces it, and what would leak without it.}{5}
\begin{solution}[title={Solution}]
...
\begin{itemize}[leftmargin=*,nosep]
  \item Attention is all-to-all by construction, so nothing in the mechanism itself respects order
  \item The training loss would fall and the model would be useless at generation time, when the future genuinely does not exist
\end{itemize}
\end{solution}
\question{A pretrained body is fine-tuned at $10^{-3}$, the rate that worked for the from-scratch model. Predict what happens, and state the rate the lecture uses instead.}{4}
\begin{solution}[title={Solution}]
The loss rises and does not recover --- steps that size destroy what was pretrained. The lecture fine-tunes at $2\times10^{-5}$.
\begin{itemize}[leftmargin=*,nosep]
  \item A pretrained block already encodes something; a step tuned for random initialisation is far too large for it
  \item Fifty times smaller than the $10^{-3}$ that trained the model from scratch --- a step tuned for random initialisation is far too large for a block that already encodes something
\end{itemize}
\end{solution}
\question{A vectoriser is fitted before the split on a 400-document corpus. The leaky vocabulary is measurably larger --- 60,000 columns against 54,074. State what that buys the model, and why.}{4}
\begin{solution}[title={Solution}]
Nothing measurable: 0.30 points against a seed spread of 2.53\%, with the leak ahead on 10 of 20 seeds. A null result.
\begin{itemize}[leftmargin=*,nosep]
  \item A term occurring only in test documents has an all-zero column in the training rows, so logistic regression gives it coefficient exactly zero --- there is no gradient to move it
  \item What actually leaks is the inverse document frequencies, and an idf is an average over the corpus: the leak changes the scale of features the model already had, and a regularised linear model is nearly indifferent to that
\end{itemize}
\end{solution}
...
\end{itemize}
\end{solution}
\question{BM25 has three parts. Name the failure of raw term counting each one answers, then say which part the ablation prices on its own as costliest, and which part it cannot price alone.}{5}
\begin{solution}[title={Solution}]
...
\begin{itemize}[leftmargin=*,nosep]
  \item Removing idf alone costs 0.1261 NDCG@10 and removing length normalisation alone costs 0.0120 --- those are the two the ablation isolates
  \item The remaining row drops saturation and length together for 0.1809, and a measurement that removes two things at once cannot attribute the loss to either of them
\end{itemize}
\end{solution}
...
\end{itemize}
\end{solution}
\question{The same model scores HR@10 of 0.0926 and 0.8102 on the same data. Name the two decisions that differ, and which moves the number more.}{5}
\begin{solution}[title={Solution}]
...
\begin{itemize}[leftmargin=*,nosep]
  ...
  \item The norms vary by 1.32$\times$ across this catalogue for no semantic reason --- a vector's length is whatever the last linear layer happened to scale it to --- so an unnormalised score ranks partly by how loudly an encoder spoke
\end{itemize}
\end{solution}
...
\end{itemize}
\end{solution}
\question{Give the five rules the course ends on, and state the property the deck points out that all five share.}{5}
\begin{solution}[title={Solution}]
...
\begin{itemize}[leftmargin=*,nosep]
  \item Every one governs what a number may be compared with, not how a model is fitted --- which is why none of them needed any
  \item They are the five that would have caught every row of the catalogue of silent failures, each of which runs to completion and raises nothing
\end{itemize}
\end{solution}
