...
\begin{failbox}[title={The weights come with their own preprocessing}]
...
\begin{failbox}[title={Freezing weights does not freeze a network}]
...
\begin{failbox}[title={Catastrophic forgetting}]
...
\begin{failbox}[title={An augmented evaluation set is not a measurement}]
...
\begin{failbox}[title={What these numbers are not}]
