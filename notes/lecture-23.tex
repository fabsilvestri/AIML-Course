...
Two encoders trained apart gave 1.0\%. Two trained together give 72.5\%. What
...
point: the same model, embeddings and $\tau$ score 0.058 at $B = 8$ and 0.835
...
\end{tabular}
\end{center}
A spread of 2.9\% from changing the wording alone. \emph{A ``zero-shot'' number
...
72.5\% at rank 1 means 27.5\% of queries did not get the right entry first, and
...
reported. The spread across the three was 2.9\%, so the optimism is of that
...
is semantic. Target zero rather than $-1$ for unrelated pairs, because on
$S^{511}$ random directions have cosine spread $1/\sqrt d$ --- predicted
...
Jointly trained, the same measurement gives R@1 of 72.5\% against 1.0\%, and
zero-shot classification reaches 90.5\% on a corpus the model has never seen
--- for one particular prompt, out of three that span 2.9\%.
